...
var _roundModifier = function _roundModifier2(v2) {
  ...
  return function(raw) {
    var n = _roundPrecise(Math.round(parseFloat(raw) / v2) * v2 * p2);
...
var snap = function snap2(snapTo, value) {
  ...
  return _conditionalReturn(value, !isArray ? _roundModifier(snapTo) : _isFunction(snapTo) ? function(raw) {
    ...
    var x2 = parseFloat(is2D ? raw.x : raw), y2 = parseFloat(is2D ? raw.y : 0), min = _bigNum, closest = 0, i = snapTo.length, dx, dy;
    while (i--) {
      if (is2D) {
        dx = snapTo[i].x - x2;
        dy = snapTo[i].y - y2;
...
var interpolate = function interpolate2(start, end, progress, mutate) {
  ...
  if (!func) {
    var isString = _isString(start), master = {}, p2, i, interpolators, l2, il;
...
var splitColor = function splitColor2(v2, toHSL, forceAlpha) {
  var a = !v2 ? _colorLookup.black : _isNumber(v2) ? [v2 >> 16, v2 >> 8 & _255, v2 & _255] : 0, r, g2, b2, h, s, l2, max, min, d, wasHSL;
  if (!a) {
    if (v2.substr(-1) === ",") {
    ...
    if (_colorLookup[v2]) {
      ...
      if (v2.length < 6) {
        ...
        g2 = v2.charAt(2);
        b2 = v2.charAt(3);
        v2 = "#" + r + r + g2 + g2 + b2 + b2 + (v2.length === 5 ? v2.charAt(4) + v2.charAt(4) : "");
      }
      if (v2.length === 9) {
  ...
  if (toHSL && !wasHSL) {
    ...
    if (max === min) {
      ...
      h = max === r ? (g2 - b2) / d + (g2 < b2 ? 6 : 0) : max === g2 ? (b2 - r) / d + 2 : (r - g2) / d + 4;
      h *= 60;
    }
    a[0] = ~~(h + 0.5);
    a[1] = ~~(s * 100 + 0.5);
...
var _ticker = function() {
  ...
  _self = {
    ...
    add: function add(callback, once, prioritize) {
      var func = once ? function(t, d, f2, v2) {
        callback(t, d, f2, v2);
        _self.remove(func);
      } : callback;
...
var Tween = function(_Animation2) {
  ...
  function Tween2(targets, vars, position, skipInherit) {
    ...
    var _this3$vars = _this3.vars, duration = _this3$vars.duration, delay = _this3$vars.delay, immediateRender = _this3$vars.immediateRender, stagger = _this3$vars.stagger, overwrite = _this3$vars.overwrite, keyframes = _this3$vars.keyframes, defaults2 = _this3$vars.defaults, scrollTrigger = _this3$vars.scrollTrigger, yoyoEase = _this3$vars.yoyoEase, parent = vars.parent || _globalTimeline, parsedTargets = (_isArray(targets) || _isTypedArray(targets) ? _isNumber(targets[0]) : "length" in vars) ? [targets] : toArray(targets), tl, i, copy, l2, p2, curTarget, staggerFunc, staggerVarsToMerge;
...
var _applySVGOrigin = function _applySVGOrigin2(target, origin, originIsAbsolute, smooth, matrixArray, pluginToAddPropTweensTo) {
  ...
  if (!originIsAbsolute) {
    ...
    y2 = xOrigin * (-b2 / determinant) + yOrigin * (a / determinant) - (a * ty - b2 * tx) / determinant;
    xOrigin = x2;
    yOrigin = y2;
  }
...
var _parseTransform = function _parseTransform2(target, uncache) {
  ...
  var style = target.style, invertedScaleX = cache.scaleX < 0, px = "px", deg = "deg", cs = getComputedStyle(target), origin = _getComputedProperty(target, _transformOriginProp) || "0", x2, y2, z2, scaleX, scaleY, rotation, rotationX, rotationY, skewX, skewY, perspective, xOrigin, yOrigin, matrix, angle, cos, sin, a, b2, c, d, a12, a22, t1, t2, t3, a13, a23, a33, a42, a43, a32;
  ...
  if (matrix !== _identity2DMatrix) {
    ...
    if (matrix.length === 6) {
      ...
      if (cache.svg) {
        ...
        y2 -= yOrigin - (xOrigin * b2 + yOrigin * d);
      }
    } else {
      a32 = matrix[6];
      a42 = matrix[7];
...
_forEachName("padding,margin,Width,Radius", function(name, index) {
  var t = "Top", r = "Right", b2 = "Bottom", l2 = "Left", props = (index < 3 ? [t, r, b2, l2] : [t + l2, t + r, b2 + r, b2 + l2]).map(function(side) {
...
// node_modules/three-story-controls/dist/three-story-controls.esm.min.js
var l = class {
  constructor(t) {
...
var g = { [u.X]: new Vector3(1, 0, 0), [u.Y]: new Vector3(0, 1, 0), [u.Z]: new Vector3(0, 0, 1) };
var b = { [u.X]: { [p.Pan]: u.X, [p.Tilt]: u.Z, [p.Roll]: u.Y }, [u.Y]: { [p.Pan]: u.Y, [p.Tilt]: u.X, [p.Roll]: u.Z }, [u.Z]: { [p.Pan]: u.Z, [p.Tilt]: u.Y, [p.Roll]: u.X } };
var y = class extends EventDispatcher {
  constructor(t, e) {
    super(), this.inTransit = false, this.upAxis = u.Y, this.actionAxes = b[this.upAxis], this.hasAnimation = false, this.animationTranslationObjectName = "Translation", this.animationRotationObjectName = "Rotation", this.translateAlong = { [p.Tilt]: false, [p.Pan]: true, [p.Roll]: false }, this.camera = t, this.scene = e, this.body = new Object3D(), this.head = new Object3D(), this.eyes = new Object3D(), this.head.name = this.animationRotationObjectName, this.body.name = this.animationTranslationObjectName, this.body.rotation.order = this.getRotationOrder(), this.head.rotation.order = this.getRotationOrder(), this.eyes.rotation.order = this.getRotationOrder(), this.scene.add(this.body.add(this.head.add(this.eyes.add(this.camera)))), this.cameraIsInRig = true, this.unpackTransform();
  }
  getAxisFor(t) {
  ...
  do(t, e, i) {
    ...
    switch (t) {
      ...
      case p.Truck: {
        const t2 = this.getAxisVectorFor(p.Tilt);
        (n || this.body).translateOnAxis(t2, e);
        break;
      }
      case p.Pedestal: {
        const t2 = this.getAxisVectorFor(p.Pan);
        (n || this.body).translateOnAxis(t2, e);
        break;
      }
      case p.Dolly: {
        const t2 = this.getAxisVectorFor(p.Roll);
        (n || this.body).translateOnAxis(t2, e);
        break;
      }
      case p.Zoom:
  ...
  setWorldCoordinates({ position: t, quaternion: e }) {
    const i = new Euler().setFromQuaternion(e, this.getRotationOrder()), s = [p.Pan, p.Tilt, p.Roll];
  ...
  flyTo(t, e, i = 1, s = "power1", o = true) {
    if (!this.isMoving()) {
      const a = this.getWorldCoordinates(), r = { px: a.position.x, py: a.position.y, pz: a.position.z, qx: a.quaternion.x, qy: a.quaternion.y, qz: a.quaternion.z, qw: a.quaternion.w, slerpAmt: 0 }, h = { px: t.x, py: t.y, pz: t.z, qx: e.x, qy: e.y, qz: e.z, qw: e.w, slerpAmt: 1 }, d = new Quaternion(), l2 = new Quaternion(r.qx, r.qy, r.qz, r.qw), p2 = () => {
        this.inTransit = true, this.packTransform(), this.dispatchEvent({ type: "CameraMoveStart" });
      }, m2 = (t2) => {
        this.body.position.set(r.px, r.py, r.pz), o ? (d.slerpQuaternions(l2, e, r.slerpAmt), this.head.setRotationFromQuaternion(d)) : this.head.quaternion.set(r.qx, r.qy, r.qz, r.qw), this.dispatchEvent({ type: "CameraMoveUpdate", progress: t2.progress() });
      }, u2 = () => {
        this.inTransit = false, this.unpackTransform(), this.dispatchEvent({ type: "CameraMoveEnd" });
      };
      gsapWithCSS.to(r, Object.assign(Object.assign({ duration: i, ease: s }, h), { onStart: p2, onUpdate: function() {
        m2(this);
      }, onComplete: u2 }));
    }
  }
  flyToKeyframe(t, e = 1, i = "power1") {
    if (this.hasAnimation && !this.isMoving()) {
      ...
      gsapWithCSS.to(s, Object.assign(Object.assign({ duration: e, ease: i }, n), { onStart: o, onUpdate: function() {
        a(this);
      }, onComplete: r }));
    }
  }
...
var v = class extends EventDispatcher {
  constructor() {
    super();
  }
};
var f = { keyMapping: { forward: ["ArrowUp", "w", "W"], backward: ["ArrowDown", "s", "S"], left: ["ArrowLeft", "a", "A"], right: ["ArrowRight", "d", "D"], up: ["u", "U"], down: ["n", "N"] }, dampingFactor: 0.5, incrementor: 1, preventBubbling: true };
var x = class extends v {
  constructor(t) {
    ...
    for (const t2 in this.keyMapping)
      e[t2] = 0;
    this.damper = new l({ values: e, dampingFactor: this.dampingFactor }), this.onKeyUp = this.onKeyUp.bind(this), this.onKeyDown = this.onKeyDown.bind(this);
  }
  connect() {
  ...
  update() {
    "continuous" !== this.type || this.damper.reachedTarget() || (this.damper.update(), this.dispatchEvent({ type: "update", values: this.damper.getCurrentValues(), deltas: this.damper.getDeltaValues() }), this.damper.reachedTarget() && (this.damper.resetAll(0), this.dispatchEvent({ type: "inertiacomplete" })));
  }
  isEnabled() {
...
var w = { domElement: document.body, dampingFactor: 0.5, shouldNormalize: true, normalizeAroundZero: true, multipointerThreshold: 100 };
var E = class extends v {
  ...
  update(t) {
    this.pointerCount !== this.cache.length && t - this.lastDownTime > this.multipointerThreshold && t - this.lastUpTime > this.multipointerThreshold && (this.pointerCount = this.cache.length, 0 === this.pointerCount ? (this.damper.resetAll(null), this.recordedPosition = false) : (this.damper.resetData(this.getPointerPosition(this.cache[0])), this.recordedPosition = true)), this.damper.reachedTarget() || (this.damper.update(), this.dispatchEvent({ type: "update", values: this.shouldNormalize ? this.normalize(this.damper.getCurrentValues(), this.normalizeAroundZero) : this.damper.getCurrentValues(), deltas: this.shouldNormalize ? this.normalize(this.damper.getDeltaValues(), false) : this.damper.getDeltaValues(), pointerCount: this.pointerCount }), this.damper.reachedTarget() && this.dispatchEvent({ type: "inertiacomplete" }));
  }
  isEnabled() {
    return this.connected;
  }
  ...
  onPointerMove(t) {
    this.pointerCount === this.cache.length && (0 === this.cache.length ? this.recordedPosition ? this.damper.setTarget(this.getPointerPosition(t)) : (this.damper.resetData(this.getPointerPosition(t)), this.recordedPosition = true) : t.pointerId === this.cache[0].pointerId && this.damper.setTarget(this.getPointerPosition(t)));
  }
  onPointerDown(t) {
...
var P = { startOffset: "0px", endOffset: "0px", buffer: 0.1, dampingFactor: 0.5 };
...
var A = { domElement: document.body, thresholdX: 60, thresholdY: 60 };
...
var L = { dampingFactor: 0.5, thresholdX: 15, thresholdY: 15, debounceDuration: 700 };
var S = class extends v {
  constructor(t) {
    super(), this.lastThresholdTrigger = 0, Object.assign(this, L, t), this.damper = new l({ values: { x: 0, y: 0 }, dampingFactor: this.dampingFactor }), this.onWheel = this.onWheel.bind(this);
  }
  connect() {
  ...
  onWheel(t) {
    ...
    else if ("discrete" === this.type && (Math.abs(t.deltaX) >= this.thresholdX || Math.abs(t.deltaY) >= this.thresholdY)) {
...
var I = { domElement: document.body, pointerDampFactor: 0.3, pointerScaleFactor: 4, keyboardDampFactor: 0.5, keyboardScaleFactor: 0.5, wheelDampFactor: 0.25, wheelScaleFactor: 0.05, panDegreeFactor: Math.PI / 4, tiltDegreeFactor: Math.PI / 10 };
var k = class {
  constructor(t, e = {}) {
    this.enabled = false, this.cameraRig = t, this.wheelScaleFactor = e.wheelScaleFactor || I.wheelScaleFactor, this.pointerScaleFactor = e.pointerScaleFactor || I.pointerScaleFactor, this.panDegreeFactor = e.panDegreeFactor || I.panDegreeFactor, this.tiltDegreeFactor = e.tiltDegreeFactor || I.tiltDegreeFactor, this.keyboardAdaptor = new x({ type: "continuous", dampingFactor: e.keyboardDampFactor || I.keyboardDampFactor, incrementor: e.keyboardScaleFactor || I.keyboardScaleFactor }), this.wheelAdaptor = new S({ type: "continuous", dampingFactor: e.wheelDampFactor || I.wheelDampFactor, domElement: e.domElement || I.domElement }), this.pointerAdaptor = new E({ domElement: e.domElement || I.domElement, dampingFactor: e.pointerDampFactor || I.pointerDampFactor }), this.onWheel = this.onWheel.bind(this), this.onKey = this.onKey.bind(this), this.onPointer = this.onPointer.bind(this);
  }
  isEnabled() {
    return this.enabled;
  }
...
var F = { startOffset: "0px", endOffset: "0px", dampingFactor: 1, buffer: 0.1, cameraStart: "0%", cameraEnd: "100%", scrollActions: [] };
var O = (t, e, i, s, n) => Math.max(s, Math.min(n, (n - s) / (i - e) * (t - e) + s));
var R = class {
  constructor(t, e) {
    this.enabled = false, this.cameraRig = t, this.cameraRig.setAnimationTime(0), this.scrollAdaptor = new T({ scrollElement: e.scrollElement, dampingFactor: e.dampingFactor || F.dampingFactor, startOffset: e.startOffset || F.startOffset, endOffset: e.endOffset || F.endOffset, buffer: e.buffer || F.buffer }), this.cameraStart = e.cameraStart || F.cameraStart, this.cameraEnd = e.cameraEnd || F.cameraEnd, this.scrollActions = e.scrollActions || F.scrollActions, this.buffer = e.buffer || F.buffer, this.calculateStops(), this.onScroll = this.onScroll.bind(this);
  }
  isEnabled() {
    return this.enabled;
  }
  ...
  onScroll(t) {
    ...
    e >= this.cameraBufferedStartPx && e <= this.cameraBufferedEndPx && this.cameraRig.setAnimationPercentage(O(e, this.cameraStartPx, this.cameraEndPx, 0, 1)), this.scrollActions.forEach((t2) => {
      e >= t2.bufferedStartPx && e <= t2.bufferedEndPx && t2.callback(O(e, t2.startPx, t2.endPx, 0, 1));
    });
  }
};
var M = { cycle: false, useKeyboard: true };
var D = class extends EventDispatcher {
  constructor(t, e = [], i = {}) {
...
var q = { wheelThreshold: 15, swipeThreshold: 60, duration: 1, ease: "power1", useKeyboard: true };
var z = class extends EventDispatcher {
  constructor(t, e = [], i = {}) {
...
var U = { domElement: document.body, panFactor: Math.PI / 20, tiltFactor: Math.PI / 20, truckFactor: 1, pedestalFactor: 1, dampingFactor: 0.7 };
var V = class {
  constructor(t, e = {}) {
    this.enabled = false, this.cameraRig = t, Object.assign(this, U, e), this.pointerAdaptor = new E({ domElement: e.domElement || U.domElement, dampingFactor: e.dampingFactor || U.dampingFactor }), this.onPointerMove = this.onPointerMove.bind(this);
  }
  isEnabled() {
    return this.enabled;
  }
...
var K = ["none", "power1", "power2", "power3", "power4", "sine", "expo", "circ"];
var j = "visit";
var Y = "remove";
var W = "duration";
var B = "ease";
...
var Z = class {
  ...
  update(t) {
    if (this.doCapture) {
      const t2 = document.createElement("canvas"), e = t2.getContext("2d");
      t2.width = 640, t2.height = 360, e.drawImage(this.canvas, 0, 0, t2.width, t2.height);
      const i = t2.toDataURL();
      this.addPoi(i), this.doCapture = false;
    }
    if (this.isPlaying) {
  ...
  createClip() {
    if (this.pois.length > 0) {
      const e = [], i = [], s = [], o = new Vector3(), a = new Quaternion(), l2 = 10;
      let p2 = 0;
      if (!this.pois[0].quaternion.isQuaternion && !this.pois[0].position.isVector3)
        for (let e2 = 0; e2 < this.pois.length; e2++) {
          const i2 = this.pois[e2];
          i2.quaternion = new Quaternion(i2.quaternion[0], i2.quaternion[1], i2.quaternion[2], i2.quaternion[3]), i2.position = new Vector3(i2.position[0], i2.position[1], i2.position[2]);
        }
      for (let t = 0; t < this.pois.length - 1; t++) {
        const n = this.pois[t], r = this.pois[t + 1], h = { px: n.position.x, py: n.position.y, pz: n.position.z, qx: n.quaternion.x, qy: n.quaternion.y, qz: n.quaternion.z, qw: n.quaternion.w, slerpAmount: 0 }, d = { px: r.position.x, py: r.position.y, pz: r.position.z, qx: r.quaternion.x, qy: r.quaternion.y, qz: r.quaternion.z, qw: r.quaternion.w, slerpAmount: 1, duration: r.duration, ease: r.ease }, m3 = gsapWithCSS.to(h, d);
        for (let t2 = 0; t2 < l2; t2++) {
          const d2 = r.duration * (t2 / l2);
          e.push(p2 + d2), m3.seek(d2), this.useSlerp ? a.slerpQuaternions(n.quaternion, r.quaternion, h.slerpAmount) : a.set(h.qx, h.qy, h.qz, h.qw), o.set(h.px, h.py, h.pz), a.toArray(s, s.length), o.toArray(i, i.length);
        }
        p2 += r.duration;
      }
  ...
  import() {
    if (this.fileInput) {
      ...
      this.fileInput.onchange = () => {
        t.readAsText(this.fileInput.files[0]), t.onload = (t2) => {
          const e = JSON.parse(t2.target.result);
  ...
  export({ draft: t }) {
    if (this.pois.length > 0) {
      const e = {};
      e.pois = this.pois.map((e2) => {
        const i2 = { position: [e2.position.x, e2.position.y, e2.position.z], quaternion: [e2.quaternion.x, e2.quaternion.y, e2.quaternion.z, e2.quaternion.w], duration: e2.duration, ease: e2.ease };
        return t && (i2.image = e2.image), i2;
      }), this.animationClip && (e.animationClip = AnimationClip.toJSON(this.animationClip));
      const i = "text/json;charset=utf-8," + encodeURIComponent(JSON.stringify(e)), s = document.createElement("a");
  ...
  initUI(t) {
    ...
    s.classList.add("btn-text", "export"), s.innerText = "export draft JSON", s.onclick = this.export.bind(this, { draft: true });
    const n = document.createElement("button");
    n.classList.add("btn-text", "export"), n.innerText = "export production JSON", n.onclick = this.export.bind(this, { draft: false });
  ...
  render() {
    this.domList.innerHTML = "", this.pois.forEach((t, e) => {
      ...
      c.innerText = "Easing";
      const l2 = document.createElement("select");
      l2.classList.add(B), l2.dataset.index = `${e}`;
      const p2 = K.map((e2) => {
      ...
      m2.classList.add(Y), m2.title = "Remove", m2.dataset.index = `${e}`, m2.innerText = "x";
      const u2 = document.createElement("button");
      u2.classList.add(j), u2.title = "Visit", u2.dataset.index = `${e}`, u2.innerHTML = "&rarr;";
      const g2 = document.createElement("button");
      g2.classList.add(X), g2.title = "Move up", g2.dataset.index = `${e}`, g2.innerHTML = "&uarr;";
      const b2 = document.createElement("button");
      b2.classList.add(N), b2.title = "Move down", b2.dataset.index = `${e}`, b2.innerHTML = "&darr;", o.append(m2, u2, g2, b2), a.append(h, d, c, l2), n.append(r, o), i.append(s, n, a), this.domList.appendChild(i);
    });
  }
};
export {
